...
import { from, lastValueFrom, Observable, Subscriber } from 'rxjs';
...
export class StreamService {
  constructor(
      // private readonly authFetchInterceptor: AuthFetchInterceptor,
  ...
  post<T>(url: string, data: T): Observable<T[]> {
      return new Observable<T[]>(observer => {
        ...
        //   const authResponse = this.authFetchInterceptor.intercept(request, (req: Request) => from(fetch(req)));

        //   lastValueFrom(authResponse)
        //       .then(async response => {
