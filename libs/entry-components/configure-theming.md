...
To gain a comprehensive understanding of theming configurations, it is highly recommended to consult the official [Angular Material documentation](https://material.angular.io/). Familiarizing the Material's theming and typography guides will provide crucial insights into the available customization options.
...
The theme can be configured in two ways, depending on the specific case, dependencies, and the structure of the project. It's crucial to note that it's not feasible to combine both methods; choose and stick with one approach instead.
...
- `primary`: The main color used in the application, it sets the tone for the overall theme
...
Each font related property in the configuration can have **family** and **size** values.
...
For customizing headers, check [this section](https://github.com/enigmatry/entry-blueprint/blob/master/enigmatry-entry-blueprint-app/theming-setup.#adding-material-classes)

- `hero-titles`: Define typography for h1, h2, h3, h4 elements
- `titles`: Typography for h5 and h6 material subtitles

> **_NOTE:_**  Properties from the list above are only applied if we use this method. If we do it on Native angular material way, the properties are different, based on Typography material guidelines. Check [this subsection](#1-typography-configuration) for that scenario:
...
We utilize the background value within the inputs configuration to establish the background color for **form text fields**. By default, the background property has no color explicitly defined, allowing the default Material styling or any existing CSS rules to take effect.
...
- `edge-gap` specifies control of both the left and right table paddings, affecting the **first and last cells** to achieve the desired spacing
- `padding` allows adding top, right, bottom, and left paddings for **all cells in table**. By default, there is no value.  It's worth mentioning that edge-gap padding will override padding values for the first and last table cells
...
In this approach, we carefully set up every little detail within the entry components. Angular Material offers a predefined color palette for application, covering a range of primary, accent, and warn colors.
...
All properties for native configuration are set to `null` in the default theme, meaning that they will not be applied, other related properties will take presence unless we redeclare or customize them.
...
There are a few steps to follow to implement this:
...
Next, we define our own Sass map that includes the previously defined $typography map. It is part of a broader theme definition. It consists of a general submap, with the property `typography`. It's associating the typographic styles ($typography) with the broader theme ($custom-theme), making those styles part of the overall design theme for the project.  By default, it's set to `null`.
...
By default, Angular Material expects color palettes (primary, accent, warn) with darker and lighter variations. Primary and accent are mandatory.
