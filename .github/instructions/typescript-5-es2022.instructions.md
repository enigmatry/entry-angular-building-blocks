...
- **Always declare class methods using the readonly arrow-function property syntax:**
  ...
  ```
  **Exception — framework lifecycle hooks** (e.g., Angular's `ngOnInit`, `ngOnDestroy`, `ngOnChanges`, etc.) must be declared as plain methods because the framework calls them by name through an interface. Do NOT use the arrow-function property syntax for these:
  ```ts
