...
- **Tech Stack**: Angular 21, TypeScript 5, Angular Material, RxJS, ngx-formly
- **Architecture**: Multi-project ng-packagr workspace — secondary entry-point libraries + demo app
- **Build**: `npm run entry-components:build` → `ng build @enigmatry/entry-form` → `ng build @enigmatry/demo-app`
- **Linting**: `npm run lint` (ESLint + Stylelint)
- **Testing**: `npm run automated-tests` (SCSS/sass-true only — no Angular unit tests)
- **Code style**: Arrow-function readonly methods; `inject()` DI; `@if`/`@for`/`@switch` control flow; strict TypeScript

> For project-specific review rules (configuration pattern, entry-point boundaries, signals API, etc.) invoke the `code-review` skill.
